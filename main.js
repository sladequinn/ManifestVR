...
async function init() {
    ...
    // Load the list of panoramas from a static JSON file
    await loadPanoramas();

    setupControls();

    // If we have panoramas, load the first one by default
...
function setupControls() {
    document.getElementById('applySettings').addEventListener('click', async () => {
        ...
        if (currentPanorama) {
            // Since we have no server to update config, we just apply these in memory and reload
            // In a real scenario, you'd update the config on a server or in storage.
...
async function loadPanorama(panorama, overrideConfig = {}) {
    ...
    // Apply overrides if provided (e.g. from sliders)
    ...
    panoSphereGeo = new THREE.SphereGeometry(
        config.sphereSize,
    ...
    loader.load(`panoramas/${panorama}/image.png`, function (texture) {
    ...
    loader.load(`panoramas/${panorama}/depth.png`, function (depth) {
        depth.minFilter = THREE.NearestFilter;
        depth.generateMipmaps = false;
        sphere.material.displacementMap = depth;
    }, undefined, function () {
        console.log(`No depth map found for ${panorama}, continuing without displacement.`);
    });
